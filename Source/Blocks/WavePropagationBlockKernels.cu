...
#include "Solvers/AugRieCUDASolver.hpp"
#elif defined(WITH_SOLVER_FWAVE)
#include "Solvers/FWaveCudaSolver.cuh"
#else
#error \
    "Invalid solver configuration. The cuda::WavePropagationBlock may only be used with CUDA enabled and either the fWave or the augRie solver!"
#endif
...
namespace cuda
{
     ...
     *           computeNetUpdatesKernel<<<dimGrid,dimBlock>>>( hd_, hud_, hvd_, bd_,
     ...
     *                                                          i_nx, i_ny
     *                                                        );
     *         To reduce the effect of branch-mispredictions the kernel provides optional offsets, which can be used to
     * compute the missing edges.
     *
     * {@link cuda::WavePropagationBlock::computeNumericalFluxes()} explains the coalesced memory access.
     *
    ...
    )
    {
        ...
        __shared__ RealType l_maxWaveSpeedShared[kTileSize * kTileSize]; // FIXME[epic=SWE,seq=7] UMA
        ...
        l_maxWaveSpeedShared[l_maxWaveSpeedPosition] = (RealType)0.0;

        //! index (l_cellIndexI,l_cellIndexJ) of the cell lying on the right side of the edge/above the edge where the
        //! thread works on.
        ...
        { // horizontal edges?
            ...
            augRieComputeNetUpdates(
                i_h[l_leftCellPosition],
                i_h[l_rightCellPosition],
                i_hu[l_leftCellPosition],
                i_hu[l_rightCellPosition],
                i_b[l_leftCellPosition],
                i_b[l_rightCellPosition],
                static_cast<real>(9.81),
                static_cast<real>(0.01),
                static_cast<real>(0.000001),
                static_cast<real>(0.0001),
                10,
                l_netUpdates
            );
#else
...
// compute the net-updates
            ...
            augRieComputeNetUpdates(
                i_h[l_leftCellPosition],
                i_h[l_rightCellPosition],
                i_hv[l_leftCellPosition],
                i_hv[l_rightCellPosition],
                i_b[l_leftCellPosition],
                i_b[l_rightCellPosition],
                static_cast<real>(9.81),
                static_cast<real>(0.01),
            ...
            // store the maximum wave speed in the shared array
            l_maxWaveSpeedShared[l_maxWaveSpeedPosition] = fmax(
                l_maxWaveSpeedShared[l_maxWaveSpeedPosition], l_netUpdates[4]
            );
        }
        ...
        { // if the reduction block size == 1*1 (1 cell) -> done.
            //! reduction partner for a thread
            int reductionPartner = 0;

            // split the block in the x-direction (size in x-dir. > 1) or y-direction (size in x-dir. == 1, size in
            // y-dir. > 1)
...
#warning skipping printf command, which was introduced for compute capability >= 2.0
...
#endif
#endif
            if (threadIdx.y < reductionBlockDimY && threadIdx.x < reductionBlockDimX)
            { // use only half the threads in each
              // reduction
                // execute the reduction routine (maximum)
                l_maxWaveSpeedShared[l_maxWaveSpeedPosition] = fmax(
                    l_maxWaveSpeedShared[l_maxWaveSpeedPosition], l_maxWaveSpeedShared[reductionPartner]
                );
            }
            // wait for all threads to finish
            __syncthreads();
        ...
        if (threadIdx.y == 0 && threadIdx.x == 0)
        {
             ...
             *
             *   In the 'main' part (e.g. gridDim = nx/TILE_SIZEm ny/kTileSize) the position
             *   is simply given by the blockId in x- and y-direction with a stride of gridDim.x + 1. The +1 results
             * from the speeds in the 'boundary' case, see below.
             *
             *   In the 'boundary' case, where the edges lie between the computational domain and the right/top ghost
             * layer, this is more complicated. In this case block offsets in x- and y-direction are used. The offsets
             * define how many blocks in the resp. direction have to be added to get a valid result. Computational
             * domain - right ghost layer: In this case the dimension of the grid in x-direction is 1. Computational
             * domain - top ghost layer: In this case the dimension of the grid in y-direction is 1.
             *
             *   Same Example as in cuda::WavePropagationBlock::computeNumericalFluxes(), assume the CUDA-grid/-blocks
             * has the following layout: <pre>
             *                                                          *
             *                                                         **        top ghost layer,
             ...
             *
             *   The maximum wave speed array on the device represents therefore logically a (4 * 3)-1 2D-array (-1: no
             * block on the top right). The 'main' part writes into cells 0, 1, 2, 4, 5 and 6. The 'computational domain
             * - right ghost layer' part writes into 3 and 7 with offset in x-direction = 3 The 'computational domain -
             * top ghost layer'  part writes into 8, 9, 10 with offset in y-direction = 2
             *
             */
            int l_maxWaveSpeedDevicePosition = computeOneDPositionKernel(
                i_blockOffSetX + blockIdx.x, i_blockOffSetY + blockIdx.y, max(i_blockOffSetY + 1, gridDim.y + 1)
            );
...
#warning skipping printf command, which was introduced for compute capability >= 2.0
        ...
        io_h[l_cellPosition]
            -= i_updateWidthX
                   * (i_hNetUpdatesRightD[l_netUpdatesRightPosition] + i_hNetUpdatesLeftD[l_netUpdatesLeftPosition])
               + i_updateWidthY
                     * (i_hNetUpdatesAboveD[l_netUpdatesAbovePosition] + i_hNetUpdatesBelowD[l_netUpdatesBelowPosition]);

        io_hu[l_cellPosition] -= i_updateWidthX
                                 * (i_huNetUpdatesRightD[l_netUpdatesRightPosition]
                                    + i_huNetUpdatesLeftD[l_netUpdatesLeftPosition]);

        io_hv[l_cellPosition] -= i_updateWidthY
                                 * (i_hvNetUpdatesAboveD[l_netUpdatesAbovePosition]
     ...
     * @return 1D index.
     */
    __device__ inline int computeOneDPositionKernel(const int i_i, const int i_j, const int i_ny)
    {
        return i_i * i_ny + i_j;
    }
} // namespace cuda